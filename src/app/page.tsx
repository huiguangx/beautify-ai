...
// === 背景装饰：漂浮色块 + 闪烁星星 + 掉落拼豆 ===
const BEAD_PALETTE = ['#ffb6d9', '#ffd6b0', '#b8f2d9', '#bfe3ff', '#d6c6ff', '#ffe3a3', '#ffc2c2'];

const KawaiiBackdrop: React.FC = () => {
  // 固定随机种子避免 hydration mismatch
  const beads = useMemo(() => (
  ...
  return (
    <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
      {/* 大色块 */}
      <div className="blob" style={{ top: '-5%', left: '-5%', width: '380px', height: '380px', background: '#ffb6d9', animationDelay: '0s' }} />
      <div className="blob" style={{ top: '10%', right: '-8%', width: '340px', height: '340px', background: '#d6c6ff', animationDelay: '-4s' }} />
      <div className="blob" style={{ bottom: '-5%', left: '20%', width: '400px', height: '400px', background: '#bfe3ff', animationDelay: '-8s' }} />
      <div className="blob" style={{ bottom: '15%', right: '5%', width: '280px', height: '280px', background: '#b8f2d9', animationDelay: '-2s' }} />

      {/* 掉落的拼豆 */}
      {beads.map((b, i) => (
        <span
          key={`b-${i}`}
          className="bead-rain absolute"
          style={{
            left: `${b.left}%`,
            top: 0,
            width: `${b.size}px`,
            height: `${b.size}px`,
            background: `radial-gradient(circle at 30% 25%, rgba(255,255,255,0.9), ${b.color} 70%)`,
            borderRadius: '50%',
      ...
      {sparkles.map((s, i) => (
        <svg
          ...
          style={{
            ...
            color: i % 3 === 0 ? '#ffb6d9' : i % 3 === 1 ? '#d6c6ff' : '#bfe3ff',
          }}
          viewBox="0 0 24 24"
          fill="currentColor"
        >
...
export default function Home() {
  ...
  return (
    ...
    {/* kawaii 主容器 */}
    <div className="relative min-h-screen flex flex-col kawaii-bg font-[family-name:var(--font-geist-sans)] overflow-x-hidden">
      <KawaiiBackdrop />

      {/* 紧凑 header：一行 logo + 标语 */}
      <header className="relative z-10 w-full max-w-5xl mx-auto px-4 pt-4 pb-3 sm:pt-6 sm:pb-4 flex-shrink-0">
        <div className="relative flex items-center justify-center gap-3 sm:gap-4">
          {/* 左侧装饰：旋转小花 + 闪星 */}
          <div className="hidden sm:block absolute left-[8%] top-1/2 -translate-y-1/2 pointer-events-none">
            <svg className="w-8 h-8 text-pink-300 animate-spin-slow" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2c1.3 0 2.5 1 2.5 2.3 0 .5-.1.9-.3 1.3 1.4-.5 3 .3 3.5 1.7.4 1.4-.3 2.9-1.7 3.5.4.2.8.3 1.3.3 1.3 0 2.3 1.2 2.3 2.5s-1 2.5-2.3 2.5c-.5 0-.9-.1-1.3-.3.5 1.4-.3 3-1.7 3.5-1.4.4-2.9-.3-3.5-1.7-.2.4-.3.8-.3 1.3 0 1.3-1.2 2.3-2.5 2.3s-2.5-1-2.5-2.3c0-.5.1-.9.3-1.3-1.4.5-3-.3-3.5-1.7-.4-1.4.3-2.9 1.7-3.5-.4-.2-.8-.3-1.3-.3C1.2 14.5.2 13.3.2 12s1-2.5 2.3-2.5c.5 0 .9.1 1.3.3-.5-1.4.3-3 1.7-3.5 1.4-.4 2.9.3 3.5 1.7.2-.4.3-.8.3-1.3C9.5 3 10.7 2 12 2z" />
              <circle cx="12" cy="12" r="3" fill="#fff" />
            </svg>
          </div>
          <div className="hidden md:block absolute left-[18%] top-0 pointer-events-none">
            <svg className="w-4 h-4 text-purple-300 animate-twinkle" viewBox="0 0 24 24" fill="currentColor" style={{ animationDelay: '0.6s' }}>
          ...
          <div className="hidden sm:block absolute right-[8%] top-1/2 -translate-y-1/2 pointer-events-none">
            <div className="relative w-10 h-10 flex items-center justify-center">
              <svg className="w-8 h-8 text-blue-300 animate-spin-reverse" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2c1.3 0 2.5 1 2.5 2.3 0 .5-.1.9-.3 1.3 1.4-.5 3 .3 3.5 1.7.4 1.4-.3 2.9-1.7 3.5.4.2.8.3 1.3.3 1.3 0 2.3 1.2 2.3 2.5s-1 2.5-2.3 2.5c-.5 0-.9-.1-1.3-.3.5 1.4-.3 3-1.7 3.5-1.4.4-2.9-.3-3.5-1.7-.2.4-.3.8-.3 1.3 0 1.3-1.2 2.3-2.5 2.3s-2.5-1-2.5-2.3c0-.5.1-.9.3-1.3-1.4.5-3-.3-3.5-1.7-.4-1.4.3-2.9 1.7-3.5-.4-.2-.8-.3-1.3-.3C1.2 14.5.2 13.3.2 12s1-2.5 2.3-2.5c.5 0 .9.1 1.3.3-.5-1.4.3-3 1.7-3.5 1.4-.4 2.9.3 3.5 1.7.2-.4.3-.8.3-1.3C9.5 3 10.7 2 12 2z" />
                <circle cx="12" cy="12" r="3" fill="#fff" />
              </svg>
              {/* 绕圈的小珠 */}
              <span className="absolute w-2 h-2 rounded-full animate-orbit" style={{ background: 'radial-gradient(circle at 30% 25%, #fff, #ffb6d9 70%)' }} />
            </div>
          </div>
          <div className="hidden md:block absolute right-[18%] bottom-0 pointer-events-none">
            <svg className="w-3.5 h-3.5 text-pink-400 animate-twinkle" viewBox="0 0 24 24" fill="currentColor" style={{ animationDelay: '1.2s' }}>
              <path d="M12 2l1.6 6.4L20 10l-6.4 1.6L12 18l-1.6-6.4L4 10l6.4-1.6z" />
            </svg>
          </div>
          ...
          <div className="relative animate-float-soft">
            <div className="grid grid-cols-3 gap-[3px] p-1.5 rounded-2xl bg-white/60 backdrop-blur-md shadow-lg border border-white/80">
              {['#ff9ec7', '#ffc9a0', '#ffe08a', '#b8e6ff', '#ffb6d9', '#c4b5ff', '#b8f2d9', '#ffd6b0', '#d6c6ff'].map((c, i) => (
                <span
                  key={i}
                  className="w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full"
                  style={{
                    background: `radial-gradient(circle at 30% 25%, rgba(255,255,255,0.95) 0%, ${c} 65%)`,
                    boxShadow: `inset -1px -1px 2px rgba(0,0,0,0.12), 0 1px 2px ${c}80`,
                  }}
                />
              ))}
            </div>
            {/* 装饰心 */}
            <svg className="absolute -top-2 -right-2 w-4 h-4 text-pink-400 animate-bounce" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 21s-7-4.5-7-10a4 4 0 017-2.6A4 4 0 0119 11c0 5.5-7 10-7 10z" />
            </svg>
          </div>

          <div className="flex flex-col">
            <h1 className="text-3xl sm:text-5xl font-black tracking-tight leading-none rainbow-text animate-title-bounce inline-block">
              Beautify
            </h1>
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className="text-xs sm:text-sm font-semibold text-purple-600 dark:text-pink-200 tracking-wider">
                拼豆 AI 生成
              </span>
              <span className="inline-block w-1 h-1 rounded-full bg-pink-400" />
              <span className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-300 font-medium">
                让创意落地成形
              </span>
            </div>
          </div>
          ...
          {/* 标题旁边的小星（贴近字） */}
          <svg className="absolute -top-1 left-[calc(50%+4.5rem)] sm:left-[calc(50%+6rem)] w-3 h-3 sm:w-4 sm:h-4 text-yellow-300 animate-wiggle pointer-events-none" viewBox="0 0 24 24" fill="currentColor">
            <path d="M12 2l1.6 6.4L20 10l-6.4 1.6L12 18l-1.6-6.4L4 10l6.4-1.6z" />
          </svg>
        </div>
      </header>
      ...
      <main ref={mainRef} className={`relative z-10 w-full md:max-w-4xl mx-auto px-4 pb-6 flex-1 flex flex-col items-center space-y-4 sm:space-y-5 ${!originalImageSrc ? 'justify-center py-4 sm:py-6' : ''}`}>
        {/* 示例展示条（未上传时）：原图 → 像素化效果预览 */}
        {!originalImageSrc && (
          <div className="w-full md:max-w-xl glass-card rounded-3xl px-4 py-3 sm:px-5 sm:py-4 animate-pop-in">
            <div className="flex items-center justify-center gap-2 sm:gap-3">
              {/* 示例 1：爱心 */}
              <div className="flex flex-col items-center gap-1.5 animate-sway-x" style={{ animationDelay: '0s' }}>
                <div className="grid grid-cols-6 gap-[1px] p-1 rounded-lg bg-white/70 shadow-sm border border-white">
                  {[
                    '·','#ffb6d9','·','·','#ffb6d9','·',
                    '#ffb6d9','#ff6fa8','#ffb6d9','#ffb6d9','#ff6fa8','#ffb6d9',
                    '#ffb6d9','#ff6fa8','#ff6fa8','#ff6fa8','#ff6fa8','#ffb6d9',
                    '·','#ffb6d9','#ff6fa8','#ff6fa8','#ffb6d9','·',
                    '·','·','#ffb6d9','#ffb6d9','·','·',
                    '·','·','·','#ffb6d9','·','·',
                  ].map((c, i) => (
                    <span key={i} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-[1px]" style={{ background: c === '·' ? 'transparent' : c }} />
                  ))}
                </div>
                <span className="text-[9px] sm:text-[10px] text-pink-500 font-semibold">❤️ 爱心</span>
              </div>

              {/* 箭头 */}
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-purple-400 animate-wiggle flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round">
                <path d="M5 12h14M13 6l6 6-6 6" />
              </svg>

              {/* 示例 2：星星 */}
              <div className="flex flex-col items-center gap-1.5 animate-sway-x" style={{ animationDelay: '0.4s' }}>
                <div className="grid grid-cols-6 gap-[1px] p-1 rounded-lg bg-white/70 shadow-sm border border-white">
                  {[
                    '·','·','#ffe08a','#ffe08a','·','·',
                    '·','·','#ffd24d','#ffd24d','·','·',
                    '#ffe08a','#ffd24d','#ffd24d','#ffd24d','#ffd24d','#ffe08a',
                    '·','#ffd24d','#ffd24d','#ffd24d','#ffd24d','·',
                    '·','#ffd24d','#ffe08a','#ffe08a','#ffd24d','·',
                    '#ffe08a','·','·','·','·','#ffe08a',
                  ].map((c, i) => (
                    <span key={i} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-[1px]" style={{ background: c === '·' ? 'transparent' : c }} />
                  ))}
                </div>
                <span className="text-[9px] sm:text-[10px] text-yellow-600 font-semibold">⭐ 星星</span>
              </div>

              {/* 箭头 */}
              <svg className="w-4 h-4 sm:w-5 sm:h-5 text-purple-400 animate-wiggle flex-shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5} strokeLinecap="round" strokeLinejoin="round" style={{ animationDelay: '0.3s' }}>
                <path d="M5 12h14M13 6l6 6-6 6" />
              </svg>

              {/* 示例 3：小熊 */}
              <div className="flex flex-col items-center gap-1.5 animate-sway-x" style={{ animationDelay: '0.8s' }}>
                <div className="grid grid-cols-6 gap-[1px] p-1 rounded-lg bg-white/70 shadow-sm border border-white">
                  {[
                    '#b8895c','·','·','·','·','#b8895c',
                    '#d4a574','#b8895c','·','·','#b8895c','#d4a574',
                    '·','#d4a574','#d4a574','#d4a574','#d4a574','·',
                    '·','#d4a574','#2a1810','#2a1810','#d4a574','·',
                    '·','#d4a574','#d4a574','#ff6fa8','#d4a574','·',
                    '·','·','#d4a574','#d4a574','·','·',
                  ].map((c, i) => (
                    <span key={i} className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-[1px]" style={{ background: c === '·' ? 'transparent' : c }} />
                  ))}
                </div>
                <span className="text-[9px] sm:text-[10px] text-amber-700 font-semibold">🐻 小熊</span>
              </div>
            </div>
            <p className="text-center text-[10px] sm:text-xs text-gray-500 dark:text-gray-300 mt-2">
              任意图片都能像这样 <span className="text-pink-500 font-semibold">秒变拼豆图纸</span> ✨
            </p>
          </div>
        )}

        {/* Drop Zone —— 主上传入口（拼豆元素自表达 + 奶油水彩底） */}
        <div
          onDrop={handleDrop} onDragOver={handleDragOver} onDragEnter={handleDragOver}
          onClick={isMounted ? triggerFileInput : undefined}
          role="button"
          aria-label="上传图片或 CSV 档案"
          tabIndex={0}
          onKeyDown={(e) => { if (isMounted && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); triggerFileInput(); } }}
          className={`relative group w-full md:max-w-2xl rounded-[22px] px-5 py-4 sm:px-6 sm:py-5 overflow-hidden ${isMounted ? 'cursor-pointer' : 'cursor-wait'} transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[0_14px_38px_-10px_rgba(255,120,180,0.45)]`}
          style={{
            background:
              'radial-gradient(120% 140% at 0% 0%, #ffe4f0 0%, transparent 55%),' +
              'radial-gradient(120% 140% at 100% 100%, #e4ecff 0%, transparent 55%),' +
              'linear-gradient(135deg, #fff9f3 0%, #fff4fb 50%, #f3f0ff 100%)',
            boxShadow: '0 8px 24px -8px rgba(255, 150, 200, 0.35), inset 0 1px 0 rgba(255,255,255,0.9)',
          }}
        >
          {/* 内层虚线边（不与背景冲突） */}
          <div
            className="pointer-events-none absolute inset-1.5 rounded-[18px] border border-dashed"
            style={{ borderColor: 'rgba(220, 160, 220, 0.55)' }}
            aria-hidden
          />

          {/* 右上角装饰珠子群（散落点缀） */}
          <div className="pointer-events-none absolute -top-2 right-4 flex gap-1 opacity-90" aria-hidden>
            <span className="bead-3d block w-3 h-3" style={{ ['--bead-color' as string]: '#ffd6b0' }} />
            <span className="bead-3d block w-2 h-2 translate-y-2" style={{ ['--bead-color' as string]: '#b8f2d9' }} />
            <span className="bead-3d block w-2.5 h-2.5" style={{ ['--bead-color' as string]: '#bfe3ff' }} />
          </div>

          <div className="relative flex items-center gap-4 sm:gap-5">
            {/* 左侧：9 颗拼豆拼成的 ↑ 上传箭头 —— 主题本身就是视觉 */}
            <div className="flex-shrink-0">
              <div className="relative p-2 rounded-2xl bg-white/70 shadow-sm ring-1 ring-pink-100 group-hover:ring-pink-200 group-hover:scale-[1.04] transition-all duration-300">
                <div
                  className="grid grid-cols-3 gap-[3px]"
                  aria-hidden
                >
                  {[
                    /* row 0 */ null,        '#ff6fa8', null,
                    /* row 1 */ '#ff6fa8',   '#ff6fa8', '#ff6fa8',
                    /* row 2 */ null,        '#c78bff', null,
                    /* row 3 */ null,        '#8cb6ff', null,
                  ].map((color, i) => (
                    color ? (
                      <span
                        key={i}
                        className="bead-3d w-3 h-3 sm:w-3.5 sm:h-3.5"
                        style={{ ['--bead-color' as string]: color }}
                      />
                    ) : (
                      <span key={i} className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                    )
                  ))}
                </div>
              </div>
            ...
            <div className="flex-1 min-w-0 text-left">
              <p className="text-base sm:text-lg font-bold leading-tight">
                <span
                  ...
                  style={{ backgroundImage: 'linear-gradient(90deg, #ff5a9e 0%, #b869ff 55%, #5a8dff 100%)' }}
                >
                  点击上传图片
                </span>
                <span className="font-normal text-gray-500 dark:text-gray-300 ml-1.5">或拖到这里</span>
              </p>
              <p className="text-[11px] sm:text-xs text-gray-500 dark:text-gray-300 mt-1 flex items-center gap-2">
                <span className="inline-flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-pink-400 shadow-[0_0_0_2px_rgba(255,120,180,0.2)]" />JPG
                </span>
                <span className="text-gray-300">·</span>
                <span className="inline-flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-purple-400 shadow-[0_0_0_2px_rgba(180,140,255,0.2)]" />PNG
                </span>
                <span className="text-gray-300">·</span>
                <span className="inline-flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-blue-400 shadow-[0_0_0_2px_rgba(140,180,255,0.2)]" />CSV
                </span>
              </p>
            </div>

            {/* 右侧：手绘风"→"箭头珠子 */}
            <div className="hidden sm:flex flex-shrink-0 items-center gap-[3px] pr-1 group-hover:translate-x-0.5 transition-transform duration-300" aria-hidden>
              <span className="bead-3d w-1.5 h-1.5" style={{ ['--bead-color' as string]: '#ffb6d9' }} />
              <span className="bead-3d w-2 h-2" style={{ ['--bead-color' as string]: '#d6c6ff' }} />
              <span className="bead-3d w-2.5 h-2.5" style={{ ['--bead-color' as string]: '#bfe3ff' }} />
            </div>
          </div>

          {/* 悬停时从左向右扫过的珠链光带 */}
          <span
            className="pointer-events-none absolute inset-y-0 -left-1/3 w-1/3 opacity-0 group-hover:opacity-100 transition-opacity duration-500"
            style={{
              background:
                'linear-gradient(90deg, transparent 0%, rgba(255,255,255,0.55) 50%, transparent 100%)',
              animation: 'hero-sweep 1.4s ease-out',
            }}
            aria-hidden
          />
        </div>
        ...
        {/* 特色徽章胶囊（未上传时展示） */}
        {!originalImageSrc && (
          <div className="w-full md:max-w-xl grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-2.5">
            {[
              { icon: '✨', title: 'AI 智能优化', color: 'from-pink-400 to-rose-400' },
              { icon: '🎨', title: '五家色号通用', color: 'from-purple-400 to-indigo-400' },
              { icon: '🧩', title: '手动精修', color: 'from-blue-400 to-cyan-400' },
              { icon: '📋', title: '导出采购单', color: 'from-emerald-400 to-teal-400' },
            ].map((f, i) => (
              <div
                key={f.title}
                className="glass-card rounded-2xl px-3 py-2.5 flex items-center gap-2 animate-pop-in"
                style={{ animationDelay: `${i * 80}ms` }}
              >
                <span className={`w-8 h-8 rounded-xl bg-gradient-to-br ${f.color} flex items-center justify-center text-sm shadow-sm`}>
                  {f.icon}
                </span>
                <span className="text-xs font-semibold text-gray-700 dark:text-gray-100">{f.title}</span>
              </div>
            ))}
          </div>
        )}
        ...
        {/* Tip Box */}
        {!originalImageSrc && (
          <div className="w-full md:max-w-xl glass-card rounded-2xl px-4 py-2.5 flex items-start gap-2">
            <span className="text-base leading-none mt-0.5">💡</span>
            <p className="text-xs text-purple-700 dark:text-purple-200 leading-relaxed">
              <span className="font-semibold">小贴士：</span>上传前裁剪图片让边缘贴合像素格子，成品更精致哦～
            </p>
          </div>
        )}
        ...
        {/* 三步上手流程（未上传时） */}
        {!originalImageSrc && (
          <div className="w-full md:max-w-xl glass-card-strong rounded-3xl px-4 py-4 sm:px-5 sm:py-5">
            <h3 className="text-center text-sm sm:text-base font-bold bg-gradient-to-r from-pink-500 via-purple-500 to-blue-500 bg-clip-text text-transparent mb-3 flex items-center justify-center gap-1.5">
              <svg className="w-4 h-4 text-pink-400 animate-twinkle" viewBox="0 0 24 24" fill="currentColor">
                <path d="M12 2l1.6 6.4L20 10l-6.4 1.6L12 18l-1.6-6.4L4 10l6.4-1.6z" />
              </svg>
              3 步拼出你的专属作品
              <svg className="w-4 h-4 text-purple-400 animate-twinkle" viewBox="0 0 24 24" fill="currentColor" style={{ animationDelay: '0.8s' }}>
                <path d="M12 2l1.6 6.4L20 10l-6.4 1.6L12 18l-1.6-6.4L4 10l6.4-1.6z" />
              </svg>
            </h3>
            <div className="grid grid-cols-3 gap-2 sm:gap-3">
              {[
                { n: '1', emoji: '📷', title: '上传图片', desc: '拖入或选择', color: 'from-pink-400 to-rose-400' },
                { n: '2', emoji: '✨', title: '智能生成', desc: 'AI 优化配色', color: 'from-purple-400 to-indigo-400' },
                { n: '3', emoji: '📥', title: '导出图纸', desc: '采购单齐全', color: 'from-blue-400 to-cyan-400' },
              ].map((s, i) => (
                <div
                  key={s.n}
                ...
                >
                  <div className={`relative w-10 h-10 sm:w-12 sm:h-12 rounded-2xl bg-gradient-to-br ${s.color} flex items-center justify-center text-lg sm:text-xl shadow-md mb-1.5`}>
                    <span>{s.emoji}</span>
                    <span className="absolute -top-1 -right-1 w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-white shadow text-[10px] sm:text-xs font-bold text-purple-500 flex items-center justify-center border border-purple-100">
                      {s.n}
                    </span>
                  </div>
                  <p className="text-xs sm:text-sm font-bold text-gray-800 dark:text-gray-100 leading-tight">{s.title}</p>
                  <p className="text-[10px] sm:text-xs text-gray-500 dark:text-gray-300 mt-0.5">{s.desc}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {originalImageSrc && (
          <div className="w-full flex flex-col items-center space-y-5 sm:space-y-6">
            ...
            {!isManualColoringMode && (
              /* 参数卡片（毛玻璃 + 圆润） */
              <div className="w-full md:max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 glass-card-strong p-4 sm:p-5 rounded-3xl">
                {/* Granularity Input */}
                <div className="flex-1">
                  <label htmlFor="granularityInput" className="flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1.5 sm:mb-2">
                    <span className="w-1.5 h-1.5 rounded-full bg-pink-400" />
                    横轴切割数量 <span className="text-[10px] text-gray-400 font-normal">(10-300)</span>
                  </label>
                  <div className="flex items-center gap-2">
                    <input
                      ...
                      className="w-full px-3 py-2 border-2 border-pink-200 dark:border-pink-900/40 rounded-xl text-sm focus:ring-2 focus:ring-pink-300 focus:border-pink-400 outline-none shadow-sm bg-white/90 dark:bg-gray-800/60 text-gray-900 dark:text-gray-100 transition"
                      min="10"
                      max="300"
                    />
                  </div>
                ...
                <div className="flex-1">
                    <label htmlFor="similarityThresholdInput" className="flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1.5 sm:mb-2">
                      <span className="w-1.5 h-1.5 rounded-full bg-purple-400" />
                      颜色合并阈值 <span className="text-[10px] text-gray-400 font-normal">(0-100)</span>
                    </label>
                    <div className="flex items-center gap-2">
                      <input
                        type="number"
                        ...
                        className="w-full px-3 py-2 border-2 border-purple-200 dark:border-purple-900/40 rounded-xl text-sm focus:ring-2 focus:ring-purple-300 focus:border-purple-400 outline-none shadow-sm bg-white/90 dark:bg-gray-800/60 text-gray-900 dark:text-gray-100 transition"
                        min="0"
                        max="100"
                      />
                    </div>
                ...
                <div className="sm:col-span-2 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="btn-candy h-9 px-4 rounded-xl bg-gradient-to-r from-pink-400 to-rose-400 text-white text-sm font-semibold whitespace-nowrap"
                  >
                    ✓ 应用数字
                  </button>
                  <button
                    ...
                    className="btn-candy inline-flex items-center justify-center h-9 px-4 text-sm font-semibold rounded-xl bg-gradient-to-r from-purple-400 to-indigo-400 text-white disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
                  >
                  ...
                  <button
                    ...
                    className="btn-candy inline-flex items-center justify-center h-9 px-4 text-sm font-semibold rounded-xl bg-gradient-to-r from-sky-400 to-cyan-400 text-white disabled:opacity-40 disabled:cursor-not-allowed whitespace-nowrap"
                  >
                    🪄 一键去背景
                  </button>
                </div>
                ...
                <div className="sm:col-span-2">
                  <label htmlFor="pixelationModeSelect" className="flex items-center gap-1.5 text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1.5 sm:mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <select
                      ...
                      className="w-full px-3 py-2 border-2 border-blue-200 dark:border-blue-900/40 rounded-xl text-sm focus:ring-2 focus:ring-blue-300 focus:border-blue-400 outline-none shadow-sm bg-white/90 dark:bg-gray-800/60 text-gray-900 dark:text-gray-100 transition"
                    >
                      <option value={PixelationMode.Dominant}>🎨 卡通（主色，色块纯净）</option>
                      <option value={PixelationMode.Average}>📷 真实（平均，过渡细腻）</option>
                    </select>
                  </div>
                </div>
                ...
                <div className="sm:col-span-2">
                  <button
                    ...
                    className="flex items-center justify-between w-full text-xs sm:text-sm font-semibold text-gray-700 dark:text-gray-200 mb-1.5 sm:mb-2"
                  >
                    ...
                    <div className="space-y-3">
                      <div className="flex flex-wrap gap-2">
                        {colorSystemOptions.map(option => (
                          <button
                            ...
                            className={`px-3.5 py-1.5 text-xs sm:text-sm font-semibold rounded-full transition-all duration-200 flex-shrink-0 ${
                              selectedColorSystem === option.key
                                ? 'bg-gradient-to-r from-pink-400 to-purple-400 text-white shadow-md transform scale-105'
                                : 'bg-white/70 dark:bg-gray-700/50 text-gray-700 dark:text-gray-200 border border-gray-200 dark:border-gray-600 hover:border-pink-300 hover:bg-pink-50 dark:hover:bg-gray-700'
                            }`}
                          >
                            {option.name}
                          </button>
                      ...
                      <div className="mt-3">
                        <button
                          ...
                          className="btn-candy w-full py-2.5 px-3 flex items-center justify-center gap-2 bg-gradient-to-r from-fuchsia-400 via-pink-400 to-orange-300 text-white font-semibold rounded-2xl"
                        >
                          <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                            <path fillRule="evenodd" d="M4 2a2 2 0 00-2 2v11a3 3 0 106 0V4a2 2 0 00-2-2H4zm1 14a1 1 0 100-2 1 1 0 000 2zm5-1.757l4.9-4.9a2 2 0 000-2.828L13.485 5.1a2 2 0 00-2.828 0L10 5.757v8.486zM16 18H9.071l6-6H16a2 2 0 012 2v2a2 2 0 01-2 2z" clipRule="evenodd" />
                          </svg>
                          管理我的色板 ({Object.values(customPaletteSelections).filter(Boolean).length} 色)
                        </button>
                        {isCustomPalette && (
                          <p className="text-xs text-center text-pink-500 dark:text-pink-300 mt-1.5 font-medium">✨ 当前使用自定义色板</p>
                        )}
                      </div>
                    </div>
                  )}
            ...
            <div className="w-full md:max-w-2xl">
              ...
              {isManualColoringMode && mappedPixelData && gridDimensions && (
                <div className="w-full mb-4 px-4 py-2.5 glass-card rounded-2xl">
                  <div className="flex flex-wrap justify-center items-center gap-2 text-xs text-gray-600 dark:text-gray-200">
                    <span className="inline-flex items-center gap-1">
                      <svg className="h-3.5 w-3.5 text-pink-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
              ...
              <div className="glass-card-strong p-4 rounded-3xl">
                {/* 大画布提示信息 */}
                {gridDimensions && gridDimensions.N > 100 && (
                  <div className="mb-3 p-2 bg-pink-50/80 dark:bg-pink-900/20 border border-pink-200 dark:border-pink-800/40 rounded-2xl text-xs text-pink-700 dark:text-pink-200 text-center">
                    <span>🔍 高精度网格 ({gridDimensions.N}×{gridDimensions.M}) — 可左右滑动查看细节</span>
                  </div>
                )}
                <div className="flex justify-center mb-3 sm:mb-4 bg-gradient-to-br from-pink-50/60 via-white/60 to-purple-50/60 dark:from-gray-800/40 dark:to-gray-900/40 p-2 rounded-2xl overflow-x-auto overflow-y-hidden kawaii-scroll"
        ...
        {!isManualColoringMode && originalImageSrc && colorCounts && Object.keys(colorCounts).length > 0 && (
          <div className="w-full md:max-w-2xl mt-2 glass-card-strong p-4 rounded-3xl color-stats-panel">
            <h3 className="text-base font-bold text-center bg-gradient-to-r from-pink-500 to-purple-500 bg-clip-text text-transparent">
              🎨 去除杂色
            </h3>
            <p className="text-xs text-center text-gray-500 dark:text-gray-300 mb-3 mt-0.5">
              点击颜色将它<span className="text-pink-500 font-semibold">排除</span> · 共 <span className="font-bold text-purple-500">{totalBeadCount}</span> 颗
            </p>
            <ul className="space-y-1 max-h-60 overflow-y-auto pr-2 text-sm kawaii-scroll">
                  ...
                  return (
                    <li
                      ...
                      className={`flex items-center justify-between px-2 py-1.5 rounded-xl cursor-pointer transition-all ${
                        isExcluded
                          ? 'bg-red-50/70 dark:bg-red-900/30 opacity-60'
                          : 'hover:bg-pink-50/70 dark:hover:bg-pink-900/20 hover:translate-x-1'
                      }`}
                    ...
                    >
                      <div className={`flex items-center space-x-2 ${isExcluded ? 'line-through' : ''}`}>
                        <span
                          className="inline-block w-5 h-5 rounded-full flex-shrink-0 shadow-sm"
                          style={{
                            background: isExcluded ? '#cbd5e0' : `radial-gradient(circle at 30% 25%, rgba(255,255,255,0.85), ${colorHex} 65%)`,
                            boxShadow: `inset -1px -1px 2px rgba(0,0,0,0.15)`
                          }}
                        />
                        <span className={`font-mono font-semibold text-xs ${isExcluded ? 'text-red-500' : 'text-gray-800 dark:text-gray-100'}`}>{displayColorKey}</span>
                      </div>
                      <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${isExcluded ? 'bg-red-100 text-red-500' : 'bg-purple-100/70 dark:bg-purple-900/40 text-purple-600 dark:text-purple-200'}`}>
                        ×{count}
                      </span>
                    </li>
                  );
            ...
            {excludedColorKeys.size > 0 && (
                <div className="mt-3">
                  <button
                    ...
                    className="w-full text-xs py-2 px-3 bg-pink-50/70 dark:bg-pink-900/20 text-pink-600 dark:text-pink-200 rounded-xl hover:bg-pink-100 dark:hover:bg-pink-900/30 transition-colors flex items-center justify-between font-semibold"
                  >
                  ...
                  {showExcludedColors && (
                    <div className="mt-2 rounded-2xl p-2 bg-gray-50/70 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700">
                      <div className="max-h-40 overflow-y-auto kawaii-scroll">
                        {Array.from(excludedColorKeys).length > 0 ? (
                          <ul className="space-y-1">
                            {Array.from(excludedColorKeys).sort(sortColorKeys).map(hexKey => {
                              ...
                              return (
                                <li key={hexKey} className="flex justify-between items-center p-1.5 hover:bg-white/80 dark:hover:bg-gray-700/50 rounded-xl">
                                  <div className="flex items-center space-x-2">
                                    <span
                                      className="inline-block w-4 h-4 rounded-full flex-shrink-0 shadow-sm"
                                      style={{ background: `radial-gradient(circle at 30% 25%, rgba(255,255,255,0.85), ${colorData?.hex || hexKey} 65%)` }}
                                    />
                                    <span className="font-mono text-xs text-gray-800 dark:text-gray-100">{getColorKeyByHex(hexKey, selectedColorSystem)}</span>
                                  </div>
                                  <button
                                    onClick={() => {
                                    ...
                                    className="text-xs py-0.5 px-2.5 bg-gradient-to-r from-pink-400 to-purple-400 text-white rounded-full font-semibold shadow-sm hover:shadow-md transition"
                                  >
                                    恢复
                                  </button>
                                </li>
                          ...
                          <p className="text-xs text-center text-gray-500 dark:text-gray-300 py-2">
                            没有排除的颜色
                          </p>
                        )}
                      </div>
                      ...
                      <button
                        ...
                        className="btn-candy mt-2 w-full text-xs py-1.5 px-3 bg-gradient-to-r from-pink-400 to-purple-400 text-white rounded-full font-semibold"
                      >
                        ✨ 一键恢复所有颜色
                      </button>
                    </div>
        ...
        {/* Message if palette becomes empty (Also hide in manual mode) */}
         {!isManualColoringMode && originalImageSrc && activeBeadPalette.length === 0 && excludedColorKeys.size > 0 && (
             <div className="w-full md:max-w-2xl glass-card p-4 rounded-2xl text-center text-sm text-amber-700 dark:text-amber-200 border border-amber-200 dark:border-amber-700/50">
                 ⚠️ 可用颜色过少或为空，请恢复部分排除色或更换色板。
                 {excludedColorKeys.size > 0 && (
                      <button
                          onClick={() => {
                          ...
                          className="mt-2 ml-2 text-xs py-1 px-3 bg-amber-200 dark:bg-amber-700/60 text-amber-900 dark:text-amber-100 rounded-full font-semibold hover:bg-amber-300 transition-colors"
                      >
                          查看已排除 ({excludedColorKeys.size})
                      </button>
                  )}
             </div>
         )}
        ...
        {!isManualColoringMode && originalImageSrc && mappedPixelData && gridDimensions && (
            <div className="w-full md:max-w-2xl space-y-3">
             ...
             <button
                ...
                className="btn-candy w-full py-3 px-4 text-sm sm:text-base font-semibold rounded-2xl flex items-center justify-center gap-2 bg-gradient-to-r from-sky-400 via-blue-400 to-indigo-400 text-white"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
             ...
             <button
                ...
                className="btn-candy w-full py-3 px-4 text-sm sm:text-base font-semibold rounded-2xl flex items-center justify-center gap-2 bg-gradient-to-r from-purple-400 via-fuchsia-400 to-pink-400 text-white"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                   <path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                   <path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                </svg>
                 🎯 专心拼豆模式 <span className="text-[10px] font-normal opacity-80">(α)</span>
             </button>
            </div>
        ...
        {!isManualColoringMode && originalImageSrc && mappedPixelData && (
            <div className="w-full md:max-w-2xl">
              <button
                ...
                className="btn-candy relative w-full py-3.5 px-4 bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 text-white text-sm sm:text-base font-bold rounded-2xl focus:outline-none disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2"
               >
      ...
      {/* 粉彩 Footer */}
      <footer className="relative z-10 w-full md:max-w-4xl mx-auto px-4 mt-4 mb-4 sm:mt-6 sm:mb-5 py-3 sm:py-4 text-center text-xs sm:text-sm flex-shrink-0">
        <div className="glass-card rounded-3xl py-5 px-4">
          <div className="flex flex-wrap gap-3 justify-center mb-3">
            <button
              onClick={() => setIsDonationModalOpen(true)}
              className="btn-candy px-5 py-2 bg-gradient-to-r from-pink-400 to-rose-400 text-white rounded-full text-xs sm:text-sm font-semibold flex items-center gap-1.5"
            >
              <span>🧋</span> 请作者喝奶茶
            </button>
            ...
            <button
              ...
              className="btn-candy px-5 py-2 bg-gradient-to-r from-purple-400 to-indigo-400 text-white rounded-full text-xs sm:text-sm font-semibold flex items-center gap-1.5"
            >
              <span>📖</span> 使用教程
            </button>
          </div>

          <p className="font-medium text-gray-600 dark:text-gray-300 flex items-center justify-center gap-1.5">
            <span className="inline-block w-2 h-2 rounded-full bg-gradient-to-br from-pink-400 to-purple-400" />
            Beautify 拼豆生成器 · {new Date().getFullYear()}
            <span className="inline-block w-2 h-2 rounded-full bg-gradient-to-br from-blue-400 to-teal-400" />
          </p>
        </div>
      </footer>
